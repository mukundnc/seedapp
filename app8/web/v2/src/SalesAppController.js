...
SalesAppController.prototype.getTreeText = function(apiRes){
	if(apiRes.success){		
		var prodSupp = apiRes.query.product.values.concat(apiRes.query.supplier.values);
		var arr = [];
		prodSupp.forEach(function(ps){
			arr.push(strToFirstUpper(ps))
		});
		return arr.join('-');
	}
	return 'No Results';
...
SalesAppController.prototype.getControllerForSearch = function(apiRes){
	if(apiRes.query.product.values.length > 1 || apiRes.query.supplier.values.length > 1){			
