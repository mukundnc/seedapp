	...
	for(var i = 0 ; i < timeItems.length ; i++){
		...
		if(this.options.dateDist === 'yearly'){
			if(itemDate.getFullYear().toString() === label){
				switch(this.options.mode){
					case 'count' : return {count : item.doc_count, value : yScale(item.doc_count)}; 
					case 'spend' : return {count : item.amount, value : yScale(item.amount)};
					case 'average_spend' : return {count : item.doc_count, value : yScale(item.doc_count)};
				}
