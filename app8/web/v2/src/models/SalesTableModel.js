...
SalesTableModel.prototype.getModel = function(uiObjects, options){
	...
	if(tables.length === 2){
		var regionTypes = ['countries', 'cities', 'suppliers'];
...
SalesTableModel.prototype.getRowsForColumn3l = function(uiColObj, xStart, yStart){
	...
	uiRows.forEach((function(b){
		var yScale = this.getYscaleForRow(b.items[0].items).yScale;
		var yScaleAmt = this.getYscaleForRow(b.items[0].items).yScaleAmt;
		rows.push({
			x : rX,
			y : rY,
			w : this.rowW,
			...
			yScale : yScale,
			yScaleAmt : yScaleAmt,
			cells : this.getCellsForRow3l(b, yScale, yScaleAmt )
		});
		rY += this.rowH;
	}).bind(this));
...
SalesTableModel.prototype.getCellsForRow3l = function(uiRowObj, yScale, yScaleAmt){
...
SalesTableModel.prototype.getYscaleForRow = function(uiCellObj){
	...
	dE = d3.max(allValsAmt,  function(v) {
				return v;
			    });
	var yScaleAmt = d3.scale.linear()
						 .domain([dS, dE])
						 .range([rS, rE]);
	return {
		yScale : yScale,
...
SalesTableModel.prototype.getColumn2l = function(uiColObj, xStart, yStart){
	var yScale = this.getYscaleForRow(uiColObj.items[0].items).yScale;
	var yScaleAmt = this.getYscaleForRow(uiColObj.items[0].items).yScaleAmt;
	return {
		key : uiColObj.key,
		count : uiColObj.doc_count,
		amount : uiColObj.amount,
		x : xStart,
		y : yStart,
		h : this.colH,
		w : this.colW,
		yScale : yScale,
		yScaleAmt : yScaleAmt,
		cells : this.getCellsForRow3l(uiColObj, yScale, yScaleAmt)
	}
}

SalesTableModel.prototype.getTableFor1Level1 = function(uiTableObj){
	var yScale = this.getYscaleForRow(uiTableObj.items).yScale;
	var yScaleAmt = this.getYscaleForRow(uiTableObj.items).yScaleAmt;
	var cells = this.getCellsForRow1l(uiTableObj, yScale, yScaleAmt);	
	return {
		tableTitle : uiTableObj.key,
		yScale : yScale,
		yScaleAmt : yScaleAmt,
...
SalesTableModel.prototype.getCellsForRow1l = function(uiRowObj, yScale, yScaleAmt){
	var cX = this.cellStartX;
	var cY = this.cellStartY;
